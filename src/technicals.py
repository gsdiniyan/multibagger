"""
Technical analysis for India — live NSE daily price history via Dhan
(bare trading symbol, no suffix needed — Dhan's own scrip master resolves it).

Sequential fetch with a polite delay between calls.
...
from src.dhan_client import get_daily_history as _dhan_daily_history
...
# ------------------------------------------------------------------ #
# Fetch via Dhan (live NSE data, bare symbol — no suffix needed)
# ------------------------------------------------------------------ #

def _fetch_history(symbol: str, retries: int = 1) -> tuple[pd.DataFrame | None, str]:
    """Fetch ~3y of daily OHLC for an NSE symbol from Dhan."""
    for attempt in range(retries + 1):
        try:
            data = _dhan_daily_history(symbol, years=LOOKBACK_YEARS)
        ...
        if data is None or data.empty:
            if attempt < retries:
                time.sleep(2)
                continue
            return None, "no data returned from Dhan (symbol not in scrip master, or empty response)"
...
def fetch_technicals(symbols: list[str], **_kwargs) -> pd.DataFrame:
    """
    Sequential Dhan fetch + indicator computation for NSE tickers.
    Each `symbol` is a bare NSE symbol — Dhan's scrip master resolves it.
    """
    n = len(symbols)
    print(f"Fetching technicals for {n} candidates via Dhan, sequential {FETCH_DELAY}s delay")
